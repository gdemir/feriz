<h4 class="page-title">Distribütörler</h4>

<table id="example" class="table table-striped table-hover dt-responsive" cellspacing="0" width="100%">
  <thead>
    <tr>
      <th>İd</th>
      <th>Bölge</th>
      <th>Ad</th>
      <th>Oluştu</th>
      <th>Düzenlendi</th>
      <th></th>
    </tr>
  </thead>
  <tbody>
    <?php if ($distributors) { ?>
    <?php foreach ($distributors as $distributor) { ?>

    <tr>
      <td><?= $distributor->id; ?></td>
      <td><?= $distributor->region->name; ?></td>
      <td><?= $distributor->name; ?></td>
      <td><?= $distributor->created_at; ?></td>
      <td><?= $distributor->updated_at; ?></td>

      <td>
        <form action="/admin/distributors/destroy" method="post">
          <a href="/admin/distributors/show/<?= $distributor->id; ?>"
            class="btn btn-default" role="button" title="Göster"><i class="fa fa-search"></i>
          </a>

          <a href="/admin/distributors/edit/<?= $distributor->id; ?>"
            class="btn btn-default" role="button" title="Düzenle"><i class="fa fa-edit"></i>
          </a>

          <input type="hidden" value="<?= $distributor->id; ?>" id="id" name="id"/>
          <button type="submit" class="btn btn-default" onClick="return confirm('Bu kaydı silmek istediğinizden emin misiniz?');" title="Sil">
            <i class="fa fa-trash"></i>
          </button>
        </form>
      </td>
    </tr>

    <?php } ?>
    <?php } else { ?>
    <tr class="text-center"><td colspan="4">Henüz Distribütör mevcut değil</td></tr>
    <?php } ?>
  </tbody>
</table>
<a class="btn btn-primary" href="/admin/distributors/create">Distribütör Ekle</a>